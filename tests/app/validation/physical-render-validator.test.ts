...
describe("validatePhysicalRender", () => {
  it("validates MP4 metadata, captions, and every requested visual checkpoint", async () => {
    const report = await validatePhysicalRender({
      request: {
        ...
        checkpointsMs: [0, 1_000, 1_999],
      },
      mediaProbe: probe,
      frameProbe: {
        capture: async (_videoPath, checkpoints) =>
          checkpoints.map((atMs) => ({ atMs, bytes: 1, sha256: "fixture-frame" })),
      },
  ...
  it("reports profile, narration, caption, and missing-frame failures without invoking quality evaluation", async () => {
    const report = await validatePhysicalRender({
      ...
      frameProbe: { capture: async () => [{ atMs: 0, bytes: 0, sha256: "fixture-frame" }] },
      readText: async () => "",
    });
  ...
  it.each([
    ...
    const report = await validatePhysicalRender({
      ...
      mediaProbe: {
        async probe() {
          return { durationMs: 1_000, hasVideo: true, hasAudio, videoCodec: "h264", width, height };
        },
      },
      frameProbe: { capture: async () => [{ atMs: 0, bytes: 10, sha256: "fixture-frame" }] },
      readText: async () => "1\n00:00:00,000 --> 00:00:01,000\nHello\n",
    });
    expect(report.status).toBe("passed");
  });
