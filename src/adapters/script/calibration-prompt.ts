...
For every click, type, hover, or zoom step, selector is REQUIRED and must be a non-empty selector copied exactly from the provided flow-graph subset. Never invent, infer, or omit a selector for those actions. Only use selectors and routes present in the provided flow-graph subset.

When the brief asks the viewer to perform an interaction (for example, a how-to, walkthrough, login, search, creation, selection, or submission) and the relevant flow-graph subset contains selector evidence, the storyboard MUST include at least one executable action: navigate, click, type, or hover. A pause-only storyboard is not a walkthrough; use pause only to support an actual interaction or explain its result. Return structured output matching the provided JSON schema exactly.`;
