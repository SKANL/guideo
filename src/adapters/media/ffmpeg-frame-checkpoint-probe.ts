...
export type FfmpegFrameExec = (
  binary: string,
  argv: readonly string[],
) => Promise<FfmpegFrameExecResult>;
...
export class FfmpegFrameCheckpointProbe implements FrameCheckpointProbe {
  constructor(
    private readonly exec: FfmpegFrameExec = defaultExec,
  ...
  ): Promise<readonly FrameCheckpoint[]> {
    const frames = await this.extract(videoPath, checkpointsMs);
    return frames.map(({ atMs, bytes }) => ({
      atMs,
  ...
  async extract(
    videoPath: string,
    checkpointsMs: readonly number[],
  ): Promise<readonly { readonly atMs: number; readonly bytes: Uint8Array }[]> {
    return Promise.all(
      checkpointsMs.map(async (atMs) => {
        const argv = [
          "-v",
          "error",
          "-ss",
